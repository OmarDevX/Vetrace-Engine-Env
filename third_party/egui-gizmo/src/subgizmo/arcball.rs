...
pub(crate) struct ArcballState { last_pos: Pos2 }
impl WidgetData for ArcballState {}
#[derive(Default, Debug, Copy, Clone)]
pub(crate) struct Arcball;
impl SubGizmoKind for Arcball { type Params = (); type State = ArcballState; }
...
impl SubGizmo for ArcballSubGizmo {
    fn pick(&mut self, ui: &Ui, ray: Ray) -> Option<f64> {
        ...
        if !pick_result.picked { return None; }
        self.update_state_with(ui, |state: &mut ArcballState| { state.last_pos = ray.screen_pos; });
        Some(pick_result.t)
    }
    ...
    fn update(&mut self, ui: &Ui, ray: Ray) -> Option<GizmoResult> {
        ...
        Some(GizmoResult { scale: self.config.scale.as_vec3().into(), rotation: crate::dquat_to_quat(new_rotation).into(), translation: self.config.translation.as_vec3().into(), mode: GizmoMode::Rotate, value: None })
    }
...
pub(crate) fn arcball_radius(config: &GizmoConfig) -> f64 { (config.scale_factor * (config.visuals.gizmo_size + config.visuals.stroke_width - 5.0)) as f64 }
