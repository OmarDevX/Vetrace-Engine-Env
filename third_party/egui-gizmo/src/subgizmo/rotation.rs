...
pub(crate) struct RotationParams { pub direction: GizmoDirection }
#[derive(Default, Debug, Copy, Clone)]
pub(crate) struct RotationState { start_axis_angle: f32, start_rotation_angle: f32, last_rotation_angle: f32, current_delta: f32 }
#[derive(Default, Debug, Copy, Clone)]
pub(crate) struct Rotation;
impl SubGizmoKind for Rotation { type Params = RotationParams; type State = RotationState; }
...
impl SubGizmo for RotationSubGizmo {
    fn pick(&mut self, ui: &Ui, ray: Ray) -> Option<f64> {
        ...
        let angle = if self.direction == GizmoDirection::View {
            ...
            if config.left_handed { forward *= -1.0; }
        ...
        if dist_from_gizmo_edge <= config.focus_distance as f64 && angle.abs() < arc_angle(self) { Some(t) } else { None }
    }
    ...
    fn update(&mut self, ui: &Ui, _ray: Ray) -> Option<GizmoResult> {
        ...
        if config.snapping { rotation_angle = round_to_interval(rotation_angle - state.start_rotation_angle as f64, config.snap_angle as f64) + state.start_rotation_angle as f64; }
        let mut angle_delta = rotation_angle - state.last_rotation_angle as f64;
        if angle_delta > PI { angle_delta -= TAU; } else if angle_delta < -PI { angle_delta += TAU; }
        self.update_state_with(ui, |state: &mut RotationState| { state.last_rotation_angle = rotation_angle as f32; state.current_delta += angle_delta as f32; });
        let new_rotation = DQuat::from_axis_angle(gizmo_normal(&self.config, self.direction), -angle_delta) * self.config.rotation;
        Some(GizmoResult { scale: self.config.scale.as_vec3().into(), rotation: crate::dquat_to_quat(new_rotation).into(), translation: self.config.translation.as_vec3().into(), mode: GizmoMode::Rotate, value: Some((gizmo_normal(&self.config, self.direction).as_vec3() * state.current_delta).to_array()) })
    }
    ...
    fn draw(&mut self, ui: &Ui) {
        ...
        let painter = Painter3d::new(ui.painter().clone(), config.view_projection * transform, config.viewport);
        ...
        if !self.active {
            ...
            let end_angle = start_angle + state.current_delta as f64 + 1e-5;
            painter.polyline(&[DVec3::new(start_angle.cos() * radius, 0.0, start_angle.sin() * radius), DVec3::ZERO, DVec3::new(end_angle.cos() * radius, 0.0, end_angle.sin() * radius)], stroke);
            ...
            if config.snapping {
                ...
                for i in 0..((TAU / config.snap_angle as f64) as usize + 1) {
                    ...
                    painter.line_segment(pos * radius * 1.1, pos * radius * 1.2, (stroke_width, stroke.1));
                }
            }
        }
    }
...
fn arc_angle(subgizmo: &SubGizmoConfig<Rotation>) -> f64 {
    let dot = gizmo_normal(&subgizmo.config, subgizmo.direction).dot(subgizmo.config.view_forward()).abs();
    let min_dot = 0.990;
    let max_dot = 0.995;
    let mut angle = f64::min(1.0, f64::max(0.0, dot - min_dot) / (max_dot - min_dot)).mul_add(FRAC_PI_2, FRAC_PI_2);
    if (angle - PI).abs() < 1e-2 { angle = PI; }
    angle
}
...
fn rotation_matrix(subgizmo: &SubGizmoConfig<Rotation>) -> DMat4 {
    ...
    if config.local_space() { rotation = config.rotation * rotation; }
...
fn rotation_angle(subgizmo: &SubGizmoConfig<Rotation>, ui: &Ui) -> Option<f64> {
    ...
    let delta = DVec2::new(cursor_pos.x as f64 - gizmo_pos.x as f64, cursor_pos.y as f64 - gizmo_pos.y as f64).normalize();
    if delta.is_nan() { return None; }
    let mut angle = f64::atan2(delta.y, delta.x);
    if subgizmo.config.view_forward().dot(gizmo_normal(&subgizmo.config, subgizmo.direction)) < 0.0 { angle *= -1.0; }
    Some(angle)
}

fn tangent(subgizmo: &SubGizmoConfig<Rotation>) -> DVec3 {
    let mut tangent = match subgizmo.direction { GizmoDirection::X | GizmoDirection::Y => DVec3::Z, GizmoDirection::Z => -DVec3::Y, GizmoDirection::View => -subgizmo.config.view_right() };
    if subgizmo.config.local_space() && subgizmo.direction != GizmoDirection::View { tangent = subgizmo.config.rotation * tangent; }
    tangent
}

fn arc_radius(subgizmo: &SubGizmoConfig<Rotation>) -> f64 {
    if subgizmo.direction == GizmoDirection::View { outer_circle_radius(&subgizmo.config) } else { (subgizmo.config.scale_factor * subgizmo.config.visuals.gizmo_size) as f64 }
}
