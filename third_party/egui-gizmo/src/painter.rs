...
impl Painter3d {
    pub fn new(painter: egui::Painter, mvp: DMat4, viewport: Rect) -> Self {
        Self { painter: painter.with_clip_rect(viewport), mvp, viewport }
    }
    ...
    fn arc_points(&self, radius: f64, start_angle: f64, end_angle: f64) -> Vec<Pos2> {
        ...
        let step_size = if step_count <= 1 { 0.0 } else { angle / (step_count - 1) as f64 };
        ...
        points.into_iter().filter_map(|point| self.vec3_to_pos2(point)).collect::<Vec<_>>()
    }

    pub fn arc(&self, radius: f64, start_angle: f64, end_angle: f64, stroke: impl Into<Stroke>) {
        let stroke: Stroke = stroke.into();
        let mut points = self.arc_points(radius, start_angle, end_angle);
        if points.len() < 2 { return; }
        let closed = points.first().zip(points.last()).filter(|(first, last)| first.distance(**last) < 1e-2).is_some();
        if closed {
            points.pop();
    ...
    pub fn circle(&self, radius: f64, stroke: impl Into<Stroke>) { self.arc(radius, 0.0, TAU, stroke); }

    pub fn filled_circle(&self, radius: f64, color: Color32) {
        let mut points = self.arc_points(radius, 0.0, TAU);
        if points.len() > 2 {
            points.pop();
            self.painter.add(Shape::convex_polygon(points, color, Stroke::NONE));
        }
    }
    ...
    pub fn arrow(&self, from: DVec3, to: DVec3, stroke: impl Into<Stroke>) {
        ...
        if let Some((start, end)) = arrow_start.zip(arrow_end) {
            let v = end - start;
            let len = v.length().max(1.0e-6);
            let cross = egui::vec2(-v.y, v.x) * (stroke.width / len);
            self.painter.add(Shape::convex_polygon(vec![start - cross, start + cross, end], stroke.color, Stroke::NONE));
        }
    }

    pub fn polygon(&self, points: &[DVec3], fill: impl Into<Color32>, stroke: impl Into<Stroke>) {
        let stroke: Stroke = stroke.into();
        let points = points.iter().filter_map(|pos| world_to_screen(self.viewport, self.mvp, *pos)).collect::<Vec<_>>();
        if points.len() > 2 { self.painter.add(Shape::convex_polygon(points, fill, stroke)); }
    }

    pub fn polyline(&self, points: &[DVec3], stroke: impl Into<Stroke>) {
        let stroke: Stroke = stroke.into();
        let points = points.iter().filter_map(|pos| world_to_screen(self.viewport, self.mvp, *pos)).collect::<Vec<_>>();
        if points.len() > 1 { self.painter.add(Shape::line(points, stroke)); }
    }

    fn vec3_to_pos2(&self, vec: DVec3) -> Option<Pos2> { world_to_screen(self.viewport, self.mvp, vec) }
}

fn steps(angle: f64) -> usize { (STEPS_PER_RAD * angle.abs()).ceil().max(2.0) as usize }
