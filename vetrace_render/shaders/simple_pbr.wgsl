...
fn vs_main(
    ...
    var n = normal;
    if (uni.has_skin != 0u) {
        p = skin_vertex(position, joints, weights);
        n = skin_normal(normal, joints, weights);
    }
...
fn vs_main_static(
    ...
    var out: VsOut;
    out.pos = uni.mvp * vec4<f32>(position, 1.0);
    out.normal = normalize((uni.model * vec4<f32>(normal, 0.0)).xyz);
    out.uv = uv;
    out.object_index = uni.object_index;
    return out;
}
