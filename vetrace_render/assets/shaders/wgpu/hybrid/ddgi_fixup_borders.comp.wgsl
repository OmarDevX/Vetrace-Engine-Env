...
fn main(@builtin(global_invocation_id) id: vec3<u32>) { if (ddgi.enabled == 0u || id.z >= ddgi.probe_counts.w) { return; } let probe = update_probe_index(id.z); let pi = max(ddgi.probe_inner_size, 1u); let di = max(ddgi.distance_inner_size, 1u); if (id.x < pi + 2u && id.y < pi + 2u && (id.x == 0u || id.y == 0u || id.x == pi + 1u || id.y == pi + 1u)) { fix_irradiance(probe, id.xy); } if (id.x < di + 2u && id.y < di + 2u && (id.x == 0u || id.y == 0u || id.x == di + 1u || id.y == di + 1u)) { fix_distance(probe, id.xy); } }
