...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let rel = (vec3<f32>(id) + vec3<f32>(0.5)) / vec3<f32>(dims);
    let world_pos = SDF_MIN + rel * SDF_SIZE;
    var rad = vec3<f32>(0.0);
    if (params.dir_light_dir.w > 0.0 && visible_to_light(world_pos)) {
        // Store conservative irradiance. SDFGI is a low-frequency volume; full
        // direct-light intensity made the nearest-voxel resolve look blocky and
        // over-colored compared with RTGI.
        rad = params.dir_light_color.xyz * params.dir_light_dir.w * 0.35;
    }
    textureStore(radiance_tex, vec3<i32>(id), vec4<f32>(clamp(rad, vec3<f32>(0.0), vec3<f32>(8.0)), 1.0));
}
