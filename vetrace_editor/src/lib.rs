...
impl EditorPlugin {
    ...
    fn render_full_editor_ui(&mut self, ctx: &egui::Context, engine: &mut Engine) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let viewport_rect = self.main_window.scene_viewport_rect(engine);
        // Always draw/update the gizmo when something is selected.  Main-panel
        // exclusion is handled by the scene viewport rect inside update_gizmo.
        // Do not gate this on `ctx.is_pointer_over_area()`: the gizmo itself is
        // an egui area, so that check can suppress the very frame where the user
        // tries to grab a handle.
...
impl Plugin for EditorPlugin {
    ...
    fn update(&mut self, engine: &mut Engine, delta_time: f32) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Scene picking belongs to the editor selection plugin. This was
        // previously initialized but never driven, so clicking in the viewport
        // could not update `selected_entities` or the renderer selection mask.
        let (mx, my) = engine.input.mouse_position();
        let pointer_in_viewport = self
            .main_window
            .scene_viewport_rect(engine)
            .contains(egui::pos2(mx as f32, my as f32));
        let wants_input = self.wants_input(engine) || !pointer_in_viewport;
        self.selection.handle_selection(
            engine,
            &mut self.main_window.selected_entities,
            wants_input,
        );
