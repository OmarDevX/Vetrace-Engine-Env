...
impl EditorPlugin {
    ...
    fn render_full_editor_ui(
        &mut self,
        ctx: &egui::Context,
        engine: &mut Engine,
    ) -> Result<(), Box<dyn std::error::Error>> {
        // Delegate main layout to the window module for a cleaner editor UI
        self.main_window.ui(ctx, &mut self.sandbox_window, engine);

        // Render transform gizmo overlay when enabled
        if self.show_gizmos {
            let hovered = self.gizmo.update_gizmo(
                engine,
                &self.main_window.selected_entities,
                self.main_window.gizmo_mode,
                self.main_window.gizmo_orientation,
            );
            self.main_window.gizmo_hovered = hovered;
        }
