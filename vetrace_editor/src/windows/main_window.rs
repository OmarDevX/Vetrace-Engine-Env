...
impl MainWindow {
    ...
    pub fn scene_viewport_rect(&self, engine: &Engine) -> egui::Rect {
        let (width, height) = engine.window.window.size();
        let mut min_x = 0.0_f32;
        let mut min_y = 0.0_f32;
        let mut max_x = width as f32;
        let mut max_y = height as f32;

        if let Some(rect) = self.left_rect {
            min_x = min_x.max(rect.max.x);
        }
        if let Some(rect) = self.right_rect {
            max_x = max_x.min(rect.min.x);
        }
        if let Some(rect) = self.top_rect {
            min_y = min_y.max(rect.max.y);
        }
        if let Some(rect) = self.bottom_rect {
            max_y = max_y.min(rect.min.y);
        }
        ...
        let resp = egui::SidePanel::left("left_panel")
            .frame(
                egui::Frame::none()
                    ...
                    .rounding(8.0)
                    .inner_margin(Margin::same(6))
                    .outer_margin(Margin { left: 10, right: 0, top: 10, bottom: 10 }),
            )
            .resizable(true)
            .max_width(350.0)
            .min_width(150.0)
        ...
        let resp = egui::SidePanel::right("right_panel")
            .frame(
                egui::Frame::none()
                    ...
                    .rounding(8.0)
                    .inner_margin(Margin::same(6))
                    .outer_margin(Margin { left: 0, right: 10, top: 10, bottom: 10 }),
            )
            .resizable(true)
            .max_width(550.0)
            .min_width(150.0)
