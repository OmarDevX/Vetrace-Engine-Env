...
pub struct ObjMesh {
    pub path: String,
    /// True after the current path has been loaded.
    pub loaded: bool,
    /// Path that produced the currently loaded mesh. Used to reload when the
    /// editor text field changes without requiring the user to toggle `loaded`.
    pub loaded_path: String,
    /// Also upload this OBJ into the ray-tracing/BVH triangle path.
    ///
    /// Keep this disabled for large visual meshes such as Sponza when you only
    /// need raster rendering, SSR, and raster shadow maps. Large OBJ triangle
    /// buffers are expensive to transform/upload every frame.
    pub raytrace: bool,
    /// Extra ECS entities spawned for OBJ material submeshes. The first submesh
    /// lives on the ObjMesh entity itself; remaining material groups are child
    /// entities so each can bind its own texture/material.
    pub submesh_entities: Vec<crate::ecs::Entity>,
}
    ...
    fn exported_fields_mut(&mut self) -> Vec<ExportedField> {
        vec![
            ...
            ExportedField {
                name: "raytrace",
                kind: ExportKind::Checkbox,
                value: &mut self.raytrace as *mut _ as *mut dyn std::any::Any,
                type_id: std::any::TypeId::of::<bool>(),
            },
        ]
    }
...
pub struct PostProcessing {
    ...
    /// Let DDGI derive a deterministic regular grid from static scene bounds.
    pub ddgi_auto_volume: bool,
    /// Manual/authored DDGI volume origin used when auto volume is disabled.
    pub ddgi_origin: [f32; 3],
    /// Manual/authored DDGI probe counts used when auto volume is disabled.
    pub ddgi_probe_counts: [u32; 3],
    /// Manual/authored DDGI spacing used when auto volume is disabled.
    pub ddgi_spacing: [f32; 3],
    /// Optional manual DDGI rays/probe override. 0 uses the quality/profile default.
    pub ddgi_rays_per_probe: u32,
    /// DDGI-specific debug view shown by the GI resolve pass. 0 disables DDGI debug output.
...
impl Default for PostProcessing {
    fn default() -> Self {
        Self {
            ...
            ddgi_auto_volume: true,
            ddgi_origin: [0.0, 0.0, 0.0],
            ddgi_probe_counts: [10, 5, 10],
            ddgi_spacing: [3.25, 3.25, 3.25],
            ddgi_rays_per_probe: 0,
            ddgi_debug_view: 0,
...
impl PostProcessing {
    ...
    pub fn indoor_60_fps() -> Self {
        Self {
            ...
            // Keep cheap raster shadows enabled. 0 disables shadows in RendererPolicy.
            shadow_quality: 1,
            max_shadow_rays: 0,
            emissive_shadow_samples: 1,
            directional_shadow_samples: 1,
...
impl Inspectable for PostProcessing {
    ...
    fn draw_ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.collapsing("Global Illumination", |ui| {
            ...
            self.raytraced_gi_enabled = self.gi_mode == GlobalIlluminationMode::RTGIOneBounce;
            if ui
                .checkbox(&mut self.path_traced_gi, "Legacy Path Traced GI")
                .changed()
            {
                self.gi_mode = if self.path_traced_gi {
                    GlobalIlluminationMode::PathTracedPreview
                } else {
                    GlobalIlluminationMode::LightProbes
                };
            ...
            if self.gi_mode == GlobalIlluminationMode::DDGI {
                ui.separator();
                ui.label("DDGI Volume Placement");
                ui.checkbox(&mut self.ddgi_auto_volume, "Auto-place from static scene bounds");
                ui.horizontal(|ui| {
                    ui.label("Origin");
                    for value in &mut self.ddgi_origin {
                        ui.add(egui::DragValue::new(value).speed(0.1));
                    }
                });
                ui.horizontal(|ui| {
                    ui.label("Counts");
                    for value in &mut self.ddgi_probe_counts {
                        ui.add(egui::DragValue::new(value).clamp_range(1_u32..=32_u32));
                    }
                });
                ui.horizontal(|ui| {
                    ui.label("Spacing");
                    for value in &mut self.ddgi_spacing {
                        ui.add(egui::DragValue::new(value).speed(0.1).clamp_range(0.25_f32..=100.0_f32));
                    }
                });
                ui.add(
                    egui::DragValue::new(&mut self.ddgi_rays_per_probe)
                        .prefix("Rays/probe override: ")
                        .clamp_range(0_u32..=512_u32),
                );

                ui.separator();
                ui.label("DDGI Debug View");
                egui::ComboBox::from_id_source("ddgi_debug_view_pp")
                    .selected_text(match self.ddgi_debug_view {
                        1 => "Irradiance",
                        2 => "Depth / Visibility",
                        3 => "Probe State",
                        4 => "Relocation Offsets",
                        5 => "Volume Coordinates",
                        6 => "Active / Fallback Status",
                        7 => "Probe Tile Index",
                        _ => "Off",
                    })
                    .show_ui(ui, |ui| {
                        ui.selectable_value(&mut self.ddgi_debug_view, 0, "Off");
                        ui.selectable_value(&mut self.ddgi_debug_view, 1, "Irradiance");
                        ui.selectable_value(&mut self.ddgi_debug_view, 2, "Depth / Visibility");
                        ui.selectable_value(&mut self.ddgi_debug_view, 3, "Probe State");
                        ui.selectable_value(&mut self.ddgi_debug_view, 4, "Relocation Offsets");
                        ui.selectable_value(&mut self.ddgi_debug_view, 5, "Volume Coordinates");
                        ui.selectable_value(&mut self.ddgi_debug_view, 6, "Active / Fallback Status");
                        ui.selectable_value(&mut self.ddgi_debug_view, 7, "Probe Tile Index");
                    });
                ui.small("The renderer profiler HUD now reports DDGI active/fallback status, volume placement, probe counts, rays/probe, update budget, atlas sizes, flags, and refresh mode.");
                ui.small("Auto mode ignores the authored origin/counts/spacing and builds a deterministic regular grid around static scene bounds.");
            }
            ...
            if ui.checkbox(&mut self.raytraced_gi_enabled, "Ray Traced GI").changed() {
                self.gi_mode = if self.raytraced_gi_enabled {
                    GlobalIlluminationMode::RTGIOneBounce
                } else {
                    GlobalIlluminationMode::LightProbes
                };
                self.path_traced_gi = false;
            }
            ui.checkbox(
            ...
            egui::ComboBox::from_id_source("rt_debug_view_pp")
                ...
                .show_ui(ui, |ui| {
                    ...
                    ui.selectable_value(&mut self.rt_debug_view, 13, "Raster Shadow Mask");
                    ui.selectable_value(&mut self.rt_debug_view, 14, "AO Contrast");
                    ui.selectable_value(&mut self.rt_debug_view, 15, "GI Buffer x5");
                });
