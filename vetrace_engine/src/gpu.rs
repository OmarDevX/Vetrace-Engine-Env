...
pub struct GpuMesh {
    ...
    pub index_count: u32,
    /// Number of vertices in `vbuf`. Hardware ray-query BLAS creation needs
    /// this because the BLAS size descriptor is expressed in vertex/index counts.
...
impl GpuMesh {
    pub fn from_cpu(
        ...
        let blas_input_usage = if device
            .features()
        ...
        let vbuf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            ...
            usage: wgpu::BufferUsages::VERTEX | blas_input_usage,
        });
        let ibuf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some(&format!("ibuf:{name}")),
            contents: bytemuck::cast_slice(indices),
            usage: wgpu::BufferUsages::INDEX | blas_input_usage,
        });
        Ok(Self {
            name: name.into(),
            vbuf,
