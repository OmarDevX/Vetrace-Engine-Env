...
impl Engine {
    pub fn new(is_2d: bool) -> Self {
        ...
        boot_log("Engine::new: before sdl2::init");
        let sdl_context = sdl2::init().unwrap();
        boot_log("Engine::new: after sdl2::init");
        boot_log("Engine::new: before WindowManager::new");
        let window = WindowManager::new(sdl_context.clone());
        boot_log("Engine::new: after WindowManager::new");
        let (width, height) = window.get_size();

        boot_log("Engine::new: before Renderer::new");
        let renderer = Renderer::new(&window.window, width as i32, height as i32, is_2d);
        boot_log("Engine::new: after Renderer::new");
        ...
        boot_log("Engine::new: before Engine struct init");
        let mut engine = Self {
            renderer,
            scene,
            physics,
        ...
        boot_log("Engine::new: after Engine struct init");
        boot_log("Engine::new: register_default_factories");
        engine.register_default_factories();
        boot_log("Engine::new: register_default_components");
        engine.register_default_components();
        boot_log("Engine::new: ensure_generated_folder");
        engine.ensure_generated_folder();
        boot_log("Engine::new: reload_scripts");
        engine.reload_scripts();
        boot_log("Engine::new: update_generated_components");
        engine.update_generated_components();
        boot_log("Engine::new: register_default_behaviours");
