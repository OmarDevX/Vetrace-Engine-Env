    ...
    pub fn run_with_behaviour<B: Behaviour + 'static>(
        ...
        while self.running {
            ...
            if !self.input.mouse_captured {
                // egui 0.34 needs a fresh pointer position even on frames where SDL
                // does not emit MouseMotion. Without this, hover/click state can lag
                // until the mouse moves again.
                self.egui_events.push(egui::Event::PointerMoved(mouse_pos));
            }

            let events: Vec<_> = self.window.poll_iter().collect();
            for event in events {
                self.input.update(&event);
                if let Some(e) = sdl_event_to_egui_event(&event) {
            ...
            let render_params = RenderParams {
                ...
                inv_view_proj: {
                    ...
                    // The raster G-buffer is written with WGPU device depth, so
                    // screen-space effects must reconstruct positions from the
                    // exact same WGPU clip transform used by the vertex passes.
                    // Without this conversion, raster shadows/SSR/GI sample a
                    // different world position and appear to slide with the camera.
                    #[cfg(feature = "wgpu")]
