...
pub fn sdl_event_to_egui_event(event: &SdlEvent) -> Option<Event> {
    use egui::{Event, PointerButton};
    match event {
        SdlEvent::MouseMotion { x, y, .. } => {
            Some(Event::PointerMoved(Pos2::new(*x as f32, *y as f32)))
        }
        SdlEvent::MouseButtonDown { mouse_btn, x, y, .. } => Some(Event::PointerButton {
        ...
        SdlEvent::MouseButtonUp { mouse_btn, x, y, .. } => Some(Event::PointerButton {
            pos: Pos2::new(*x as f32, *y as f32),
            button: match mouse_btn {
                MouseButton::Left => PointerButton::Primary,
                MouseButton::Right => PointerButton::Secondary,
...
impl Engine {
    /// Take the queued egui events and make sure the current pointer position is
    /// present in the same RawInput frame that is passed to `egui_ctx.run`.
    ///
    /// SDL only emits MouseMotion while the mouse moves, but egui still needs a
    /// fresh pointer position on idle frames for stable hover/click resolution.
    /// Do this at RawInput construction time instead of relying only on the
    /// outer event loop, because some render paths can consume or rebuild egui
    /// input independently.
    pub(crate) fn take_egui_events_for_frame(&mut self) -> Vec<Event> {
        let mut events = std::mem::take(&mut self.egui_events);

        if !self.input.mouse_captured {
            let mouse = sdl2::mouse::MouseState::new(&self.window.event_pump);
            let pos = Pos2::new(mouse.x() as f32, mouse.y() as f32);

            // Keep this first so button press/release events that follow keep
            // their own exact SDL-provided positions and ordering.
            events.insert(0, Event::PointerMoved(pos));
        }
    ...
    pub fn render_frame(&mut self) {
        ...
        let mut ddgi_auto_volume = 1u32;
        let mut ddgi_origin = [0.0_f32, 0.0, 0.0];
        let mut ddgi_probe_counts = [10_u32, 5, 10];
        let mut ddgi_spacing = [3.25_f32, 3.25, 3.25];
        let mut ddgi_rays_per_probe = 0u32;
        let mut ddgi_debug_view = 0u32;
            ...
            .query::<crate::components::components::CameraAttachment>()
        {
                ...
                .get::<crate::components::components::PostProcessing>(ent)
            {
                ...
                gi_mode = if pp.path_traced_gi {
                    ...
                    == crate::components::components::GlobalIlluminationMode::DDGI
                {
                    crate::rendering::wgpu_renderer::GI_MODE_DDGI
                } else if pp.raytraced_gi_enabled
                    || pp.gi_mode
                        == crate::components::components::GlobalIlluminationMode::RTGIOneBounce
                {
                    // Make the Ray Traced GI checkbox actually select the RTGI producer.
                    // Previously it only wrote a flag that the policy never used for GI routing.
                    crate::rendering::wgpu_renderer::GI_MODE_RTGI_ONE_BOUNCE
                } else {
                    pp.gi_mode.as_u32()
                };
                ddgi_auto_volume = pp.ddgi_auto_volume as u32;
                ddgi_origin = pp.ddgi_origin;
                ddgi_probe_counts = pp.ddgi_probe_counts;
                ddgi_spacing = pp.ddgi_spacing;
                ddgi_rays_per_probe = pp.ddgi_rays_per_probe;
                ddgi_debug_view = pp.ddgi_debug_view;
        ...
        let render_params = RenderParams {
            ...
            inv_view_proj: {
                ...
                // Match the WGPU clip/depth convention used by raster G-buffer
                // vertex passes. Otherwise reconstructed world positions drift
                // and raster shadows/SSR/GI look camera-locked or misaligned.
                #[cfg(feature = "wgpu")]
                let vp = OPENGL_TO_WGPU_MATRIX
                    * perspective(cam.fov, aspect, z_near, 1000.0)
        ...
        #[cfg(feature = "wgpu")]
        {
            use crate::components::components::{GlobalTransform, Transform};

            let (w, h) = self.renderer.screen_dimensions();
            let aspect = w as f32 / h as f32;
            let view_mat = look_at(&Vec3::ZERO, &cam_front, &cam_up);
            // Large OBJ scenes such as Sponza are wider/deeper than 1000 world units
            // after import. Use a safer far plane so they do not vanish from clipping.
            let z_far = 10000.0;
            let proj_mat = perspective(cam.fov, aspect, z_near, z_far);
            ...
            for (e, transform, mesh) in self.world.query2::<Transform, MeshHandle>() {
                ...
                let (mesh_position, mesh_orientation, mesh_size) = if let Some(gt) = self.world.get::<GlobalTransform>(e) {
                    (gt.position, gt.orientation, gt.size)
                } else {
                ...
                let model = Mat4::from_scale_rotation_translation(
                    ...
                    Vec3::from(mesh_position) - cam_pos,
                );
                let mvp = (OPENGL_TO_WGPU_MATRIX * proj_mat * view_mat * model).to_cols_array_2d();
                ...
                let object_index = self
                    .world
                    .get::<ObjectRef>(e)
                    .map_or(-1, |obj_ref| obj_ref.id as i32);
    ...
    pub fn render_ui<F>(&mut self, mut ui_callback: F)
    where
        ...
        for (i, obj) in self.scene.objects.iter_mut().enumerate() {
            ...
            let idx = if let Some(mat) = entity_mat {
                *mat_map.entry(mat.name.clone()).or_insert_with(|| {
                ...
                let base_color_factor = obj.base_color_factor();
