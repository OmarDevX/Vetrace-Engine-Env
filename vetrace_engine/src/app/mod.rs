...
impl AppBuilder {
    ...
    pub fn run<A: App>(self, mut app: A) -> Result<(), Box<dyn std::error::Error>> {
        ...
        engine.window.window.set_size(self.window_size.0, self.window_size.1);
        ...
        while engine.running {
            ...
            for event in events {
                ...
                if let Some(egui_event) = crate::engine::engine::sdl_event_to_egui_event(&event, mouse_pos) {
            ...
            // Check for basic input events
            if engine.input.is_key_pressed("Escape") {
                engine.running = false;
            }

            // Create basic input events from engine state
            if engine.input.is_key_pressed("Space") {
                input_events.push(InputEvent::KeyPressed { key: "Space".to_string() });
            }
