...
impl AppBuilder {
    ...
    pub fn run<A: App>(self, mut app: A) -> Result<(), Box<dyn std::error::Error>> {
        ...
        boot_log("AppBuilder::run: before plugin registration");
        ...
        boot_log("AppBuilder::run: before initialize_plugins");
        // Initialize plugins
        plugin_manager.initialize_plugins(&mut engine)?;
        boot_log("AppBuilder::run: after initialize_plugins");
        ...
        while engine.running {
            ...
            // Keep egui pointer state current even when SDL has no MouseMotion event.
            ...
            if !engine.input.mouse_captured {
                engine.egui_events.push(egui::Event::PointerMoved(mouse_pos));
            }
            ...
            for event in events {
                ...
                if let Some(egui_event) =
                    crate::engine::engine::sdl_event_to_egui_event(&event)
                {
