...
pub async fn init_wgpu(
    ...
    // Hardware ray query in wgpu currently needs the experimental ray-query
    // feature and, in practice, the Vulkan backend on Linux/Windows. Keep the
    // default path portable, but when VETRACE_HW_RAY_QUERY=1 is requested,
    // force Vulkan so we do not accidentally pick a backend that cannot build
    // acceleration structures.
    let hw_ray_query_requested = std::env::var("VETRACE_HW_RAY_QUERY")
        .map(|v| v == "1")
        .unwrap_or(false);
    let requested_backends = if hw_ray_query_requested {
        Backends::VULKAN
    } else {
        Backends::PRIMARY
    };
    ...
    instance_desc.flags = InstanceFlags::from_env_or_default();
    let instance = Instance::new(instance_desc);
    boot_log("init_wgpu: before create surface");
    let surface = unsafe {
        // SDL2 on Linux/Wayland/X11 can provide the window handle separately
        // from the display handle. wgpu 29 needs both; using from_window()
        // alone may build a target with no display handle and panic with
        // CreateSurfaceError::MissingDisplayHandle.
        let target = SurfaceTargetUnsafe::from_display_and_window(window, window)
            .expect("raw display/window handles");
        instance
    ...
    if ray_query_support.uses_hardware() {
        // Acceleration structure limits default to zero unless these minimums
        // are requested; BLAS/TLAS creation will panic/fail without them.
    ...
    let (device, queue) = adapter
        .request_device(&DeviceDescriptor {
            ...
            // Request the adapter's reported limits so the renderer can bind
            // large texture arrays for materials. When hardware ray query is
            // active, include the acceleration-structure minimums above.
            required_limits: limits,
            experimental_features: if ray_query_support.uses_hardware() {
