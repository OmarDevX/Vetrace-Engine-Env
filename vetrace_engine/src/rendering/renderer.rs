...
pub enum DdgiDebugView {
    ...
    RelocationOffsets = 4,
    VolumeCoordinates = 5,
    ActiveFallbackStatus = 6,
    ProbeTileIndex = 7,
}

impl Default for DdgiDebugView {
    fn default() -> Self {
        Self::Off
    }
}
...
impl Default for DdgiPlacementMode {
    fn default() -> Self {
        Self::None
    }
}
...
#[derive(Clone, Copy, Debug, Default)]
pub struct RendererDdgiStatus {
    pub requested: bool,
    pub active: bool,
    pub resources_ready: bool,
    pub fallback_active: bool,
    pub placement_mode: DdgiPlacementMode,
    pub debug_view: DdgiDebugView,
    pub refresh_mode: &'static str,
    ...
    pub origin: [f32; 3],
    pub spacing: [f32; 3],
    pub irradiance_atlas: [u32; 2],
    pub distance_atlas: [u32; 2],
    pub flags: u32,
}
...
pub struct RendererHybridFeatureStatus {
    ...
    pub ddgi_status: RendererDdgiStatus,
}
...
pub struct RenderParams {
    ...
    pub gi_mode: u32,
    /// Non-zero lets the renderer derive a deterministic DDGI volume from static scene bounds.
    pub ddgi_auto_volume: u32,
    /// Authored DDGI origin used when `ddgi_auto_volume == 0`.
    pub ddgi_origin: [f32; 3],
    /// Authored DDGI probe counts used when `ddgi_auto_volume == 0`.
    pub ddgi_probe_counts: [u32; 3],
    /// Authored DDGI spacing used when `ddgi_auto_volume == 0`.
    pub ddgi_spacing: [f32; 3],
    /// Optional authored ray count. 0 uses the quality/profile default.
    pub ddgi_rays_per_probe: u32,
    /// DDGI-specific debug view. Values match `DdgiDebugView`.
...
mod gi_routing_tests {
    ...
    fn base_params() -> RenderParams {
        RenderParams {
            ...
            shadow_mode: 0,
            raytraced_shadows_enabled: 0,
            raytraced_reflections_enabled: 0,
            raytraced_gi_enabled: 0,
            raytraced_transparency_enabled: 0,
            shadow_quality: 1,
            max_shadow_rays: 0,
            emissive_shadow_samples: 1,
            directional_shadow_samples: 1,
            cloud_object_shadows_enabled: 0,
            max_rt_shadow_distance: 50.0,
            rt_shadow_ray_t_max: 50.0,
            min_soft_shadow_radius: 0.03,
            _pad_shadow_mode: 0,
            rt_debug_view: 0,
            rt_debug_counters: 0,
            max_traversal_steps: 128,
            max_transparent_surfaces: 4,
            shadow_max_distance: 50.0,
            reflection_max_distance: 50.0,
            ...
            atmosphere: 0,
            atmosphere_mode: 0,
            atmosphere_sun_controls: [0.0; 4],
            cloud_history_weight: 0.0,
            cloud_sample_count: 0,
            cloud_temporal_quality: 0,
            cloud_shadow_mode: 0,
            renderer_mode: RendererMode::HybridEffects,
            clouds: Vec::new(),
            profile: RendererProfile::High,
        }
    }

    #[test]
    fn ddgi_routes_when_requested_and_supported() {
        let params = base_params();
        let policy = RendererPolicy::derive(
            &params,
            RendererHardwareCapabilities { ddgi: true, ..Default::default() },
            0,
            1.0,
        );
        assert_eq!(policy.gi, GiMethod::DDGI);
    }

    #[test]
    fn ddgi_falls_back_when_unsupported_or_low_budget() {
        let mut params = base_params();
        let unsupported = RendererPolicy::derive(
            &params,
            RendererHardwareCapabilities { ddgi: false, ..Default::default() },
            0,
            1.0,
        );
        assert_eq!(unsupported.gi, GiMethod::LightProbes);

        params.profile = RendererProfile::Low;
        let low_budget = RendererPolicy::derive(
            &params,
            RendererHardwareCapabilities { ddgi: true, ..Default::default() },
            0,
            1.0,
        );
        assert_eq!(low_budget.gi, GiMethod::LightProbes);
    }
}
