...
fn pos(index: u32) -> vec3<f32> { return ddgi.volume_min.xyz + vec3<f32>(grid(index)) * ddgi.volume_spacing.xyz; }
@compute @workgroup_size(64)
fn main(@builtin(global_invocation_id) id: vec3<u32>) { let probe = id.x; if (ddgi.enabled == 0u || probe >= ddgi.probe_counts.w) { return; } var min_d = ddgi.max_ray_distance; var hit_count = 0u; for (var r = 0u; r < ddgi.rays_per_probe; r = r + 1u) { let d = textureLoad(ray_distance, vec2<i32>(r, probe), 0).x; min_d = min(min_d, d); if (d < ddgi.max_ray_distance * 0.98) { hit_count = hit_count + 1u; } } let camera_relevant = distance(pos(probe), ddgi.camera_pos.xyz) < ddgi.max_ray_distance * 4.0; let geometry_relevant = hit_count > max(ddgi.rays_per_probe / 32u, 0u) && min_d > ddgi.normal_bias; probe_states[probe] = select(0u, 1u, camera_relevant || geometry_relevant); }
