            ...
            if (in_bounds_tri(ti)) {
                ...
                if (hit.t < t_cap) {
                    ...
                    if (mat.baseColorTex != 0u) {
                        ...
                        a = a * textureSampleLevel(rt_material_texture, tex_sampler, uv, 0.0).a;
                    }
...
fn hit_alpha(obj_idx: u32, tri_idx: u32, uv: vec2<f32>) -> f32 {
    ...
    if (mat.baseColorTex != 0u && tri_idx != 0xffffffffu) {
        let tex = textureSampleLevel(rt_material_texture, tex_sampler, uv, 0.0);
        a = a * tex.a;
    }
...
fn evaluate_default_material(hit: vec3<f32>, normal: vec3<f32>, mat: MaterialParams, tri_idx: u32, uv: vec2<f32>) -> MaterialResult {
    ...
    if (mat.baseColorTex != 0u && tri_idx != 0xffffffffu) {
        tex = textureSampleLevel(rt_material_texture, tex_sampler, uv, 0.0).rgb;
    }
...
fn screen_space_reflection_history(pixel: vec2<u32>, dims: vec2<u32>, hit_pos: vec3<f32>, normal: vec3<f32>, view_dir: vec3<f32>, roughness: f32) -> vec4<f32> {
    ...
    for (var i: i32 = 1; i <= REFLECT_SSR_STEPS; i = i + 1) {
        ...
        let sample_depth = textureLoad(depth_tex, sample_px).x;
