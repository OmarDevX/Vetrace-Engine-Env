...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let radius = clamp(bitcast<f32>(rt_params.rtao_radius_bits), 0.05, 1.25);
    ...
    // RTAO is a detail term; keep it from crushing indirect lighting.
    ao = mix(1.0, ao / max(weight_sum, 1.0e-4), 0.72);
    textureStore(ao_out, px, vec4<f32>(clamp(ao, 0.0, 1.0), 0.0, 0.0, 1.0));
}
