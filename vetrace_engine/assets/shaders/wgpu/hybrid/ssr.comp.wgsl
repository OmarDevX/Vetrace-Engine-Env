...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let steps = max(i32(ssr.max_steps), 1);
    let step_len = max(ssr.max_distance / f32(steps), ssr.stride) * (0.35 + roughness);
    ...
    for (var i: i32 = 1; i <= steps; i = i + 1) {
        ...
        if (depth_error < ssr.thickness + roughness * 0.12 && normal_ok > 0.25) {
            ...
            let fade = clamp(edge * 8.0, 0.0, 1.0) * (1.0 - f32(i) / f32(steps + 1));
            let current = textureLoad(current_color_tex, sp, 0).rgb;
            let prev_proj = project_to_prev_uv(sample_world);
            ...
            if (all(prev_proj.xy >= vec2<f32>(0.0)) && all(prev_proj.xy <= vec2<f32>(1.0)) && prev_proj.z >= 0.0 && prev_proj.z <= 1.0) {
                ...
                let hd = textureLoad(depth_tex, hp, 0).r;
                let hn = unpack_normal(hp);
                let hw = reconstruct_world(hp, dims, hd);
                let h_depth_error = abs(length(hw - ssr.camera_pos.xyz) - length(sample_world - ssr.camera_pos.xyz));
