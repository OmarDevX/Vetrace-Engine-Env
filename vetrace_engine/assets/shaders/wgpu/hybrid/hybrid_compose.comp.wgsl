...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if (params.rt_debug_view == 13u) {
        textureStore(color_tex, px, vec4<f32>(vec3<f32>(raster_shadow), 1.0));
        return;
    }
    ...
    if (params.rt_debug_view == 14u) {
        ...
        textureStore(color_tex, px, vec4<f32>(vec3<f32>(contrast), 1.0));
        return;
    }
    ...
    if (params.rt_debug_view == 15u) {
        ...
        textureStore(color_tex, px, vec4<f32>(min(gi * 5.0, vec3<f32>(1.0)), 1.0));
        return;
    }
