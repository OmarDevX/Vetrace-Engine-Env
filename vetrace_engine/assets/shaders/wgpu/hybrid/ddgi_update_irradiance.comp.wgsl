...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    for (var r = 0u; r < ddgi.rays_per_probe; r = r + 1u) { let ray_dir = spherical_fibonacci(r, ddgi.rays_per_probe); let w = max(dot(dir, ray_dir), 0.0); let sample = textureLoad(ray_radiance, vec2<i32>(i32(r), i32(probe)), 0).rgb; weighted = weighted + sample * w; weight_sum = weight_sum + w; }
