...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    let inner = max(ddgi.probe_inner_size, 1u); let tile = inner + 2u; let local_probe = id.z; if (ddgi.enabled == 0u || local_probe >= ddgi.probe_counts.w || id.x >= inner || id.y >= inner) { return; } let probe = update_probe_index(local_probe);
    let atlas_xy = probe_tile_origin(probe, tile, ddgi.atlas_size.x) + id.xy + vec2<u32>(1u);
    let uv = ((vec2<f32>(f32(id.x), f32(id.y)) + vec2<f32>(0.5)) / f32(inner)) * 2.0 - vec2<f32>(1.0);
    let dir = oct_decode(uv);
    var weighted = vec3<f32>(0.0); var weight_sum = 0.0;
    for (var r = 0u; r < ddgi.rays_per_probe; r = r + 1u) { let ray_dir = spherical_fibonacci(r, ddgi.rays_per_probe); let w = max(dot(dir, ray_dir), 0.0); let sample = textureLoad(ray_radiance, vec2<i32>(i32(r), i32(local_probe)), 0).rgb; weighted = weighted + sample * w; weight_sum = weight_sum + w; }
    let integrated = weighted / max(weight_sum, 1.0e-4) * DDGI_PI;
    let prev = textureLoad(previous_irradiance, vec2<i32>(i32(atlas_xy.x), i32(atlas_xy.y)), 0).rgb;
    let h = clamp(ddgi.hysteresis, 0.0, 0.98);
    textureStore(irradiance_out, vec2<i32>(i32(atlas_xy.x), i32(atlas_xy.y)), vec4<f32>(mix(integrated, prev, h), 1.0));
}
