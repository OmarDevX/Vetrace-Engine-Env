...
| Shadow maps / cascades | **Partial** | `renderer_impl.inc.rs` owns `raster_shadow_map`, `raster_shadow_view_proj`, primitive/PBR shadow passes, and reports `raster_shadow_maps_active`. | Single raster shadow-map support is wired. Cascaded shadow maps were not found in the current WGPU renderer and should be added by extending the existing raster shadow resources/passes. |
...
| RT shadows | **Partial** | `renderer_impl.inc.rs` builds and dispatches `hybrid_rt_shadow_pipeline`; shader: `assets/shaders/wgpu/experimental/hybrid_effects/rt_shadows.comp.wgsl`; monolithic path-traced shadow logic remains in `assets/shaders/wgpu/hybrid/pathtrace.comp.wgsl`. | Hybrid RT shadows are wired, but the split shader still lives under `experimental/hybrid_effects/` and should share traversal/material helpers before becoming the long-term shader home. |
| RT reflections | **Partial** | `renderer_impl.inc.rs` builds and dispatches `hybrid_rt_reflection_pipeline`; shader: `assets/shaders/wgpu/experimental/hybrid_effects/rt_reflections.comp.wgsl`; compositor: `assets/shaders/wgpu/experimental/hybrid_effects/composite.comp.wgsl`. | Hybrid RT reflections are wired as a bounded effect pass with history, while path-traced reflections remain in monolithic path tracing. |
| RTGI | **Partial** | `renderer_impl.inc.rs` builds and dispatches `hybrid_rt_gi_pipeline`; shader: `assets/shaders/wgpu/experimental/hybrid_effects/rt_gi.comp.wgsl`; GI routing is summarized in `SHADER_ARCHITECTURE.md`. | Hybrid RTGI is a one-bounce additive mode. Raster modes use cheaper GI paths, while cinematic/path-traced modes promote to monolithic path-traced GI. |
...
| Temporal accumulation | **Partial** | `renderer_impl.inc.rs` maintains history textures/copies and hybrid reflection history; `assets/shaders/wgpu/experimental/hybrid_effects/composite.comp.wgsl` blends GI history with `temporal_blend`; `assets/shaders/wgpu/hybrid/hybrid_compose.comp.wgsl` is the active hybrid composition fallback. | Temporal history exists for denoise, hybrid effects, clouds, and post-FX, but parity differs by feature and mode. Document new histories and resolve ownership before adding more accumulation buffers. |
...
| Transparency / translucency | **Partial** | `renderer_impl.inc.rs` builds/dispatches `hybrid_rt_transparency_pipeline`; shader: `assets/shaders/wgpu/experimental/hybrid_effects/rt_transparency.comp.wgsl`; compositor: `assets/shaders/wgpu/experimental/hybrid_effects/composite.comp.wgsl`. | Hybrid transparency is a split effect pass, not a complete unified translucency model across raster, hybrid, and path-traced paths. |
